...
public class MyProxyVideoProducer extends MyProxyPlugin{
	...
	public MyProxyVideoProducer(BigInteger id, ProxyVideoProducer producer){
        ...
        mWidth = MyProxyVideoProducer.DEFAULT_VIDEO_WIDTH;
		mHeight = MyProxyVideoProducer.DEFAULT_VIDEO_HEIGHT;
		mFps = MyProxyVideoProducer.DEFAULT_VIDEO_FPS;
    }
		...
		if(camera != null && mProducer != null && mVideoFrame != null){
			...
			if(MyProxyVideoProducer.addCallbackBufferSupported){
				for(int i=0; i<MyProxyVideoProducer.CALLABACK_BUFFERS_COUNT; i++){
					MyCameraProducer.addCallbackBuffer(camera, new byte[mVideoFrame.capacity()]);
				}
			}
	...
	private PreviewCallback previewCallback = new PreviewCallback() {
	  public void onPreviewFrame(byte[] _data, Camera _camera) {
		  if(MyProxyVideoProducer.super.mValid){
	...
	class MyProxyVideoProducerPreview extends SurfaceView implements SurfaceHolder.Callback {
		...
		public void surfaceCreated(SurfaceHolder holder) {
			try {
				/*final Camera camera =*/ MyCameraProducer.openCamera(myProducer.mFps, 
						myProducer.mWidth, 
						myProducer.mHeight, 
						mHolder,
